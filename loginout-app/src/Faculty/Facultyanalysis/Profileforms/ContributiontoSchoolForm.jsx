...
const ContributionForm = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const data = await authContext.getContributionToSchoolByToken();
        console.log("ss " + data);
  ...
  const handleAddRow = async () => {
    ...
    const getData1 = async () => {
      try {
        const data = await authContext.AddContributionToSchoolByToken({
          responsibility: "",
          contribution: "",
          score: 0,
        });
    ...
    const removerow = async () => {
      if (index)
        try {
          ...
          const data = await authContext.RemoveContributionToSchoolByToken(
            index
          );
          // console.log("ss " + data);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const data = await authContext.AddAllContributionToSchoolByToken(
        contributions
      );
      // console.log("ss " + data);
